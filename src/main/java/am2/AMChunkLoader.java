...
public class AMChunkLoader implements LoadingCallback{
	...
	private Ticket requestTicket(int x, int y, int z, World world){
		...
		if (ticket != null){
			cacheTicket(new TicketIdentifier(x, y, z, world.provider.getDimension()), ticket);
			return ticket;
		}
		return null;
	}
	...
	public void releaseStaticChunkLoad(Class clazz, int x, int y, int z, World world){
		Ticket ticket = getTicket(new TicketIdentifier(x, y, z, world.provider.getDimension()));
		if (ticket == null){
